...
\label{s:theories}
	...
	\label{ss:fenix}
		...
		\footnote{In \cite{fen49}*{p~3}. Honduras founded when south-east Yucatan was no longer inhabited by the Spanish ie after the Maya uprising in or Blauvelt raids of Bacalar, and founded before its territory was ceded to Wallace by the Mosquito ruler via treaty. Sierra O'Reilly uses \ment{Wallace} properly, but we read the name symbolically.}
		% re theory = none more
		For this to hold, we xx carving.%
		\footnote{Sierra O'Reilly seems to speak in terms of a collective cum corporate and spatially contiguous and temporally continuous state which emerges via an intentional act at a single temporal point.}
		% re carving ↓
	...
	\subsection{Baymen's} % may name Neo-Wallacian or Campbellian/Restallian (if follows Campbell/Restall) - or give Wallacian w/ usual sense of state and emergence but for Bc+Id, then given Campbellian w/ looser sense of state or emergence but for Bc - then commercial vs mil/legal pressures theories
	\label{ss:possyfirst}
		Honduras was a haunt-state in the inner Bay which emerged by the 1630s as early Baymen increasingly used the place for buccaneering.% possibility on - state exists if institutions exist, if inhabitants say it exists, if economic pressures call for it
		\footnote{This looks like something an early Bayman might say, and we imagine their reasoning as follows. Say buccaneers regarded the inner Bay as one whole composed of three contiguous parts (the close haunts in \ref{eqn:map}). Further say they derived their sense of ownership from use, and first used \(\id\) at \(t_i\) xx.}
	%
	%
