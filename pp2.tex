...
\label{s:problem}
	...
	\label{ss:features}
		...
		\footnote{It seems to require binary concepts (some thing is or is not a state or polity, and does or does not exist, and cannot \emph{sort of} nor \emph{partially} be one nor exist). For simplicity, require that all concepts be binary, but let \ment{state} be a strict subconcept of \ment{polity} where the latter includes things which are kind of but not properly states eg state-like entities or quasi-states (leaving the state-nonstate boundary in \ment{polity} vague or fuzzy).} % add thanks to Campbell for this being required
...
\label{s:theories}
	...
	\footnote{Literature haphazardly chosen from Table~\ref{tab:literature}. Qualify all claims regarding literature as being \emph{for} a possible or at least slightly acceptable (not an intended nor accurate nor most acceptable) reading. Possible theories also haphazardly chosen. These seem to be at least as viable as existing theories.} % may note no ordered viability claim - may note does not exhaust viable theories - may add unviable possible theories
	%
	%
	...
	\label{ss:fenix}
		...
		\footnote{In \cite{fen49}*{p~3}. Honduras founded when south-east Yucatan was no longer inhabited by the Spanish ie after the Maya uprising in or Blauvelt raids of Bacalar, and founded before its territory was ceded to Wallace by the Mosquito ruler via treaty. Sierra O'Reilly uses \ment{Wallace} properly, but we read the name symbolically, and ignore biographical details. The Maya uprising here may be the Tipu Rebellion, and the Mosquito \ment{ruler} is properly the Mosquito \ment{King}.} % may note 'Tipu Reb' sometimes names multi-year rebellion or incl Sp responses to rebellion
		% re theory = none more
	...
	\label{ss:campbell}
		...
		For this to hold, we xx carving.%
		\footnote{xx}
		% re carving ↓
	...
	\label{ss:thomases}
		...
		For this to hold, we do not seem to need a carving more realised than the one in Section~\ref{ss:carving}.%
		\footnote{The Bulmer-Thomases seem to speak in terms of a collective, intentional, continuous state with strict succession, but \emph{for} the model used here, we can imagine competing senses giving us this same theory.} % note their model is clearer to see than their carving - note cannot use nonstrict succession
		% re carving ↓
	...
	\label{ss:restall}
		...
		\footnote{In \cite{res19}*{pp~2--4, 12--13, 17, 20--21, 30}. Honduras located on and about Old [pp~12--13]. Honduras founded definitely in 1716 when regular cutting began [pp~12--13], and definitely not in 17th century [pp~3-4, 12, 17, 20--21, 30]. Cutting occurs possibly from 1662 in small occasional incursions [pp~12, 17], and is regularised definitely in 1716 [p~13]. This is one of the few theories in literature building a model from historical trend data (cartographic patterns, in this case) rather than discrete data points (though it is not immediately apparent that the chosen pattern is relevant for non-Restallian carvings).} % may note etymology/toponomy trends also explicit in some lit
		For this to hold, we need a very particular carving and xx models.%
		\footnote{xx}
