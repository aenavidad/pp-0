...
\label{s:problem}
	...
	\label{ss:features}
		...
		\footnote{We mainly have in mind here existing or possibly useful senses. These are at least the spatiotemporal vs collective vs corporate state, small vs large state, intentional vs accidental emergence of state, continuous vs discontinuous state, voluntary vs involuntary discontinuity of state, and strict vs nonstrict succession.} % may gloss these senses - may note existence kept binary for simplic (count claim 'state partly exists' as 'state exists in partial form'), but may get similar carvg for graded exist - may also keep binary succession and only grade/vary state and emergence?
...
\label{s:models}
	...
	\label{ss:temporal}
		...
		\footnote{Plus \(t_{\og}\), plus a further relation needed for \ment{lead to}. We do not plot related phenomena nor states of affairs (nor their \ment{encourage to} nor \ment{discourage from} relations) for simplicity, and because current evidence in literature does not really give us a clear picture of them nor their relations, such that their relevance is also not very clear. For instance, it is possible for commercial pressures to have had much more of a bearing on piracy or logging than legal or military ones, and vice versa, but we have not really come across solid evidence for nor against either possibility (despite the latter one seemingly being more often than not presumed in literature, judging by how often legal or military pressures are cited, as compared to commercial ones). Further, almost all of these would seem to more naturally fit a graded (as phenomena) rather than binary (as states of affairs) representation (despite the latter being more common in literature). That said, we might still end up with similar (but messier) \ref{eqn:logs} if we plotted them.} % may note record of only gain bucc for bucc not near/about Jam
...
\section{Theories}
\label{s:theories}
	In this section, we review theories (existing ones to Section~\ref{ss:restall}, and possible ones from Section~\ref{ss:possyfirst}) to see which carvings of the problem and models of the record they hold for.%
	...
	\label{ss:fenix}
		...
		\footnote{In \cite{fen49}*{p~3}. \ment{Wallace} read symbolically. Honduras founded when south-east Yucatan was no longer inhabited by the Spanish ie after the Maya uprising in or Blauvelt raids of Bacalar.}
		For this to hold, we xx carving.%
		\footnote{Sierra O'Reilly seems to speak in terms of a collective cum corporate, intentional, continuous state with strict succession.}
		% re carving ↓
	...
	\subsection{Campbell}
	\label{ss:campbell}
		Honduras was a haunt- and stand- and settlement-state in Bacalar which emerged in the 1550s (first) and by the mid 17th century (last) or by 1765 (last) as pirates and buccaneers and the British gradually went from frequenting to squatting to settling for buccaneering or cutting.%
		\footnote{In \citelist{\cite{cam03}*{pp~171, 174--176, 178--180} \cite{cam11}*{pp~vii, xii, 82--84, 90--93, 95--97, 100, 104, 106--108, 121--122, 129, 156--157}}. Honduras first emerged via formation of loose communities from probably mid 16th century \cite{cam11}*{pp~95--96, 100}, or via use as haven from probably mid 1550s \citelist{\cite{cam03}*{pp~176, 179} \cite{cam11}*{p~xii}}. Honduras continued emerging via further use as haven from early 1570s \cite{cam11}*{pp~96--97} or by 17th century \cite{cam03}*{p~174} or definitely by 1670s \citelist{\cite{cam03}*{pp~176, 178} \cite{cam11}*{pp~82--83}} or by end 17th century \cite{cam11}*{p~95}, and possibly via Providencer use from 1641 \cite{cam11}*{p~129}. Honduras last emerged via sedentary settlement by mid 17th century \cite{cam03}*{p~171} or by 1708 \cite{cam11}*{pp~91--92}, or via dissolution of Spanish administration in 1708 \cite{cam11}*{pp~vii, xii, 90--93}, or via principal or important settlement definitely from 1765 \citelist{\cite{cam03}*{pp~175, 179--180} \cite{cam11}*{121--122}}. Honduras emerged in and about St George's \cite{cam03}*{pp~171, 175}. Cutting cum buccaneering occurs possibly by Elizabethan era or by early 17th century or before 1655 \cite{cam11}*{pp~106--107}, or occurs by 1695 \cite{cam11}*{pp~84, 156--157} or by 1697 \cite{cam11}*{p~91}. Cutting sans buccaneering occurs by mid 17th century \cite{cam03}*{p~171}, and intensifies by early 1670s \cite{cam11}*{p~108} or by 1680s \cite{cam11}*{p~104} or by 1708 \cite{cam11}*{pp~91--92}. Campbell seems to say that the state emerged bit by bit such that it first \emph{fully} existed when the whole was completed (mid 17th cent/1765 for Honduras) and only \emph{partly} existed from when the first brick was laid (1550s for Honduras). We would rather keep \ment{existence} binary, and so read Campbell as saying that the state emerged first in one form, then morphed into another form, and so on, until it took on its modern form, such that it first existed when its first form appeared (1550s for Honduras).}
		For this to hold, we xx carving.%
		\footnote{xx}
		% re carving ↓
		% state partly emerges if used as haven [cam03 171, 176] or if settlement is attempted [cam11 vii] or if agents entrench themselves well [cam11 95]
		% state fully emerges if sedentary activities occur [cam03 171], or if a settlement is established [cam03 174] or a principal settlement [cam03 175 /cam11 96] or a permanent one [cam03 176] or one with a leader [cam03 176]
		xx models.%
		\footnote{xx}
		% re temporal ↓
		% cutt = of any dyewood by bucc [cam11 pp~104--105], bucc = non-Sp non-res pir pre-1620s, then res pir from 1620s, then Brit bucc
	...
	\label{ss:thomases}
		Honduras was a haunt- or stand- and settlement-state in Belize established or founded in 1642--1669 (possibly) or by the 1670s (likely) and by 1705 (definitely) when the British settled for buccaneering or cutting.%
		...
		% re carving ↓
		% state emerges if a more than transitory settlement is established [p~137], or if agents make some place their home [p~154]
	...
	\subsection{Restall}
	\label{ss:restall}
		Honduras was a stand- and settlement-state in Bacalar founded in 1717 when Campeachy men settled for cutting.%
		\footnote{In \cite{res19}*{pp~2--3, 12--13, 17, 20--21, 30}. Cutting occurs haphazardly possibly from 1662 [p~xx], and intensified definitely from 1717 [p~xx].}
		For this to hold, xx carving. xx models.
	%
	%
	% subsec possible first
	\subsection{Possible} % may name Neo-Wallacian or Campbellian/Restallian (if follows Campbell/Restall)
	\label{ss:possyfirst}
		Honduras was a haunt- and settlement-state in Bacalar and the Bay Islands which emerged in the 1630s when xx.
	%
	%
	% subsec possible second
	\subsection{Possible}
	\label{ss:possysecond}
		Honduras was a xx.
%
%
%
