...
\label{s:models}
	...
	\label{ss:temporal}
		...
		(4) &t_i \rar &t_{\ct} \rar["?"] &t_j\\
		(5) &t_i \rar["?"] &t_{\ct} \rar["?"] &t_j
		\end{tikzcd}
		\end{equation}
		
		We start by listing temporal points (and their relations) commonly represented in literature. And note these seem to be phenomenal bounds near or about \(t_{\og}\). xx In which case, we plot \emph{all} xx, and \emph{all} xx.
		
		\subsubsection{Bounds}
		We start by discarding theories which disregard the common consensus (that piracy somehow led to logging), and so are left with only ones which invoke at least piracy or logging or both.%
		...
		Now let \ment{buccaneering} and \ment{cutting} be subclasses of piracy and logging, respectively.%
		...
		Then for \(t_{\buc}\) the upper bound of buccaneering \emph{in} the Bay, and \(t_{\ct}\) the lower bound of cutting \emph{in} the Bay, we ought to and do have \(t_0 \leq t_{\buc} \leq t_{\ct} \leq t_1\) or \(t_0 \leq t_{\ct} \leq t_{\buc} \leq t_1\).%
		\footnote{For either loose or tight bounds. We restrict bounds to the Bay for simplicity, and feel this is licensed as all existing and most possible \(t_{\og}\) likewise mark an event or happening in the same. This disallows a spatially discontinuous state though, so further restricts possible carvings and theories. Note a good number of existing theories consider \emph{both} loose and tight bounds for cutting (ie light vs heavy or sporadic vs frequent cutting). For points only in the Bay, \ref{eqn:logs} now seem to have all notable or frequent points in literature (possibly missing minor or infrequent ones). For points in or about the Bay, \ref{eqn:logs} seem to be missing only one such (possibly missing minor or infrequent points), namely, the point (usually a short time period) when many or most pirates mostly dropped piracy and took up logging. This could have happened over an long period of itme though, and we would ideally want stronger evidence (than currently available in literature) to say it did so in a short period, so xx.}
		%
		%Now, then, in addition to figuring out how exactly did piracy lead to logging, we ought to see how buccaneering and logging might be specified, and whether one led to the other, and to figure out where \(t_{\og}\) is in (2) and (3) of \ref{eqn:logs}.
		
		\subsubsection{Points} % non-bound points - no need to mention/consider literature now - nor piracy/logging
		Now say some phenomenon \ment{finely} leads to another if most of the latter's earlier incidents are incidents of the former too.%
		\footnote{Appropriately qualified, eg for temporally tight but spatially loose incident bounds, for some early-late threshold, so on. We mean to count as single incidents cases where agents first set out to plunder in one place but then end up cutting in another (say because they found so no ships to surprise in the first), so on. There are ways of getting at this concept which do not rely so much on careful carving of incidents, eg in terms of closeness, so on.}
		And say they \ment{track} each other if their incidence curves are very similar.%
		\footnote{For loose similarity over \(x\) (time) and tight over \(y\) (tally) axes.}
		And lastly distinguish \ment{fruitful} from fruit\ment{less} buccaneering.%
		\footnote{Appropriately qualified, eg for some profitable-unprofitable threshold which splits buccaneering into disjoint subphenomena, so on. This distinction also available for \ment{piracy}.}
		Then fruitless buccaneering either did or did not not finely lead to cutting, and either did or did not track fruitful buccaneering.%
		\footnote{The class of theories which do not presume fruitless buccaneering finely lead to cutting might have quite a few existing members. Fruitful buccaneering's finely leading to cutting does not straightforwardly seem viable.}
		So we have at least four cases near \(t_{\ct}\).
		% ie four paths through tight t_ct or from loose t_ct - may call this section 'Paths' and say non-bound points not needed for models
%
%
%
% sec theories
